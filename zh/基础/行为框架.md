...
# 系统内置活动
UAPI系统已经内置了很多通用的公共活动可供应用程序使用，所有系统内置的行为都定义在`uapi.behavior.common.action`包下，它们是：
## 睡眠
名称：Sleep
作用：将当前活动的执行线程暂停指定的时间
## 获取服务
名称：RetrieveService
作用：根据指定的名称和初始化属性从服务仓库中获取服务，并返回该服务
## 保存对象
名称：StoreObject
作用：将指定的对象用指定的名称保存在指定的上下文范围内，并返回该对象
## 取出对象
名称：RestoreObject
作用：从指定上下文范围内取出指定名称对应的对象，并返回该对象
